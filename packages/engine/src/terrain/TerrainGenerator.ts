...
import { makeNoise2D, fbm, Noise2DFn } from './Noise'
...
export interface TerrainConfig {
  ...
  continentalWarpFrequency:    number
  continentalWarpStrength:     number
  continentalSigmoidSteepness: number
  midlandOffset:               number
  mountainOffset:              number
  tierThresholdLow:            number
  tierThresholdHigh:           number
}

export const DEFAULT_TERRAIN_CONFIG: TerrainConfig = {
  waterLevel:                  3.0,
  domainFrequency:             0.0018,
  plainsThreshold:             0.28,
  mountainThreshold:           0.52,
  domainWarpStrength:          40,
  enableRivers:                true,
  riverWidth:                  0.07,
  riverCarveDepth:             3.5,
  terraceStep:                 4.5,
  continentalFrequency:        0.0004,
  continentalWarpFrequency:    0.0003,
  continentalWarpStrength:     60,
  continentalSigmoidSteepness: 15,
  midlandOffset:               120,
  mountainOffset:              250,
  tierThresholdLow:            0.33,
  tierThresholdHigh:           0.66,
}

// ─── BiomeProvider interface ─────────────────────────────────────────────────
...
/** Minimal interface TerrainGenerator needs from the biome system. */
export interface BiomeProvider {
  getBlend(wx: number, wz: number): BiomeBlend
  getHellCenter(): { x: number; z: number } | null
  megaMountains: MegaMountain[]
}
...
function warpFbm(noise: Noise2DFn, x: number, z: number, freq: number, octaves: number, warpStrength: number): number {
  const w  = warpStrength
...
export function riverMask(wx: number, wz: number, cfg: TerrainConfig = DEFAULT_TERRAIN_CONFIG): number {
  if (!cfg.enableRivers) return 1
  ...
  return Math.min(1.0, rv / cfg.riverWidth)
}
 ...
 */
export function continentalOffset(wx: number, wz: number, cfg: TerrainConfig = DEFAULT_TERRAIN_CONFIG): number {
  const cn = getContinentalNoise()
  const tn = getTerrainNoise()

  // Domain warp for organic, meandering tier boundaries
  const wf = cfg.continentalWarpFrequency
  const ws = cfg.continentalWarpStrength
  const warpX = wx + ws * fbm(tn, wx * wf,        wz * wf + 31.4, 2)
  const warpZ = wz + ws * fbm(tn, wx * wf + 17.3, wz * wf,        2)

  // Sample continental noise (0-1)
  const raw = fbm(cn, warpX * cfg.continentalFrequency,
                      warpZ * cfg.continentalFrequency, 3)
  ...
  const k    = cfg.continentalSigmoidSteepness
  const low  = cfg.tierThresholdLow
  const high = cfg.tierThresholdHigh

  const t1 = 1 / (1 + Math.exp(-k * (spread - low)))
  const t2 = 1 / (1 + Math.exp(-k * (spread - high)))

  let offset = t1 * cfg.midlandOffset
             + t2 * (cfg.mountainOffset - cfg.midlandOffset)
...
function computeHeight(
  ...
  if (domain < cfg.plainsThreshold) {
    // ── Plains / valleys ──────────────────────────────────────────────────
    const t   = domain / cfg.plainsThreshold   // 0..1
    const flat = fbm(noise, wx * 0.05, wz * 0.05, 2) * heightScale * 0.10
    const hill = warpFbm(noise, wx, wz, heightFreq, 4, cfg.domainWarpStrength) * heightScale * 0.35
    h = flat + (hill - flat) * (t * t)

  } else if (domain < cfg.mountainThreshold) {
    // ── Rolling hills ─────────────────────────────────────────────────────
    h = warpFbm(noise, wx, wz, heightFreq, 5, cfg.domainWarpStrength) * heightScale

  } else {
    // ── Mountain ridges ───────────────────────────────────────────────────
    const mt     = (domain - cfg.mountainThreshold) / (1 - cfg.mountainThreshold)
    const hillH  = warpFbm(noise, wx, wz, heightFreq,       5, cfg.domainWarpStrength) * heightScale
  ...
  if (terraceStrength > 0 && h > cfg.waterLevel + 1) {
...
export function generateHeightmap(
  ...
  getBiomeConfig: (type: BiomeType) => BiomeConfig,
  cfg: TerrainConfig = DEFAULT_TERRAIN_CONFIG,
): HeightmapResult {
  ...
  for (let row = 0; row < VERTICES; row++) {
    for (let col = 0; col < VERTICES; col++) {
      ...
      const blend = biomeProvider.getBlend(wx, wz)
      const bA    = getBiomeConfig(blend.primary)
      const bB    = getBiomeConfig(blend.secondary)

      // Single domain sample shared by both biomes for coherence
      const domain = fbm(noise, wx * cfg.domainFrequency, wz * cfg.domainFrequency, 3)

      const hA = computeHeight(noise, domain, wx, wz,
        bA.heightScale, bA.heightFrequency, bA.mountainScale, bA.terraceStrength, bA.terraceStep, cfg)
      const hB = computeHeight(noise, domain, wx, wz,
        bB.heightScale, bB.heightFrequency, bB.mountainScale, bB.terraceStrength, bB.terraceStep, cfg)

      let height = hA + (hB - hA) * blend.blend

      // Continental elevation — large-scale tier offset
      const contOffset = continentalOffset(wx, wz, cfg)
      height += contOffset

      // Mega mountain boost — sharp conical peaks scattered across the world
      for (const mt of biomeProvider.megaMountains) {
        const mdx = wx - mt.x
        const mdz = wz - mt.z
      ...
      // Hell pit carving
      const hellC = biomeProvider.getHellCenter()
      if (hellC) {
      ...
      const localWater = contOffset + cfg.waterLevel

      // River carving
      const rm = riverMask(wx, wz, cfg)
      if (rm < 1) {
        const bed = localWater - cfg.riverCarveDepth
